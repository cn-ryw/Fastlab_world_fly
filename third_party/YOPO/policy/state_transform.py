...
class StateTransform:
    ...
    def pred_to_endstate(self, endstate_pred: torch.Tensor) -> torch.Tensor:
        ...
        # per-anchor half-range tensors [N], flipped to match the CNN-cell order (like yaw/pitch)
        yaw_diff = self.lattice_primitive.yaw_diff.flip(0)[None, :]      # [1, N]
        pitch_diff = self.lattice_primitive.pitch_diff.flip(0)[None, :]  # [1, N]
        delta_yaw = endstate_pred[:, :, 0] * yaw_diff  # [B, N]
    ...
    def pred_to_endstate_cpu(self, endstate_pred: np.ndarray, lattice_id: torch.Tensor) -> np.ndarray:
        ...
        """
        # per-anchor half-range, indexed by the selected lattice ids
        yaw_diff = self.lattice_primitive.yaw_diff[lattice_id].cpu().numpy()
        pitch_diff = self.lattice_primitive.pitch_diff[lattice_id].cpu().numpy()
        delta_yaw = endstate_pred[:, 0] * yaw_diff
        delta_pitch = endstate_pred[:, 1] * pitch_diff
        radio = (endstate_pred[:, 2] + 1.0) * self.lattice_primitive.radio_range
