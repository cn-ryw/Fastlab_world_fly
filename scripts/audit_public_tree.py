...
FORBIDDEN_PATHS = (
    ...
    re.compile(r"(^|/)drone_controller_config\.json$", re.I),
...
CONTENT_RULES = (
    ...
    (
        ...
        re.compile(
            r"(?i)\b(?:api[_-]?key|access[_-]?token|secret|password)\b\s*[:=]\s*['\"][A-Za-z0-9_./+=-]{20,}['\"]"
        ),
    ),
)
TEXT_SUFFIXES = {
...
def audit(staged=False):
    ...
    for path in tracked_paths(staged):
        ...
        if not is_text_path(path):
            continue
        try:
            text = file_bytes(path, staged).decode("utf-8")
