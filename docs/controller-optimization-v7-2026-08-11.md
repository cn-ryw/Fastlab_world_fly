...
优先解决运行时可观测性、规划时延、轨迹时间对齐和终端到达控制，再讨论策略重训。
不得用放宽 250 ms 陈旧帧门限、强行提高倾角/推力、降低避障权重或给 YOPO 活动段叠加
`15 m/s` 速度反馈来掩盖问题。

最近两次 Firefox 城市飞行显示 DA360 metric → YOPO → Poly5 → SO3 能够完成闭环到达，
...
- 只保留最新帧，不积压旧请求；
- 六面切片默认保持 2 个面一批；真实日志已证明 3 个面一批会扩大单帧阻塞，1/3/6 只保留为显式 A/B 参数；
...
当前本地回归结果为：26 个 JS 测试文件、132 个宿主 Python 测试、Docker 内 30 个后端
契约测试，以及依赖版本、JS/Python 语法和差异格式检查。50 ms Poly5 在 200 Hz
下已锁定为 10/10 个直接加速度控制样本，首个超过终点时刻的固定步才进入终端或保持逻辑。

真实 Firefox preview A/B 已完成：500 m 完整预载下，1200 m hidden-viewer far plane + lean streaming
两次的 capture p95 为 53/45 ms，RAF/physics-frame p95 均为 17.1 ms；旧 15000000 m + over-fetch
对照为 62/33.1 ms，故保留新默认值。但这些结果尚不能宣称规划实时性门禁通过。后续仍需使用
