...
已验证实现基线：`63fd9f1`（严格 bundle、授权门禁和运行时尺寸契约）。当前工作树另有 capture profile、timing/evaluator v2 与无内容哈希的运行身份改动；自动测试已复验通过（精确计数以最终 sweep 原始输出为准），但 live Firefox/GPU 与可信 15Hz 尚未验收。
...
- 原型 `c0b82d5` 已生成仓外 bundle、archive branch 和 annotated tag。
...
- 六面 capture 默认每两面向调度器 yield，支持 AbortSignal，并输出 render/project/scheduler timings；3 面实验因真实日志中单帧阻塞恶化而退回显式 A/B 参数。
...
- `/depth` 保持小 JPEG并恢复轻量 metadata；`/depth/raw` 输出 NPZ 和完整的 frame/capture、模型输入及投影契约 metadata。
...
- metric loader 绑定可读模型名、模型/请求尺寸、projection/JPEG、resample 与 accepted 标志；每请求携带 `X-Projection-Config`，并在 GPU 前核对 RGB 尺寸和完整运行时投影配置。
- `window.__captureMetricCalibration()` 从一个冻结 `PerceptionFrame` 原子导出同 captureId 的 RGB、anchors、manifest 与 raw NPZ，并记录三份文件名与字节数、session/pose/transform、ERP/JPEG/上传尺寸和模型 metadata。
- fitter 强制四件套按文件名与 session/capture/location/frame identity 配对，校验输入/投影/pose/数组语义，并要求每地点 3 个 distinct poses、每地点 LOLO 过门及 0.5–20m 范围指标。
...
- FlightLogger/evaluator schema v2 只计 `trajectoryApplied=true` 的实际轨迹安装；combined response 使用进程级 service session ID 保证单次飞行不跨服务重启，模型与配置用可读名称记录。
- evaluator v2 丢弃 30 个合法唯一 warmup frames，再要求 ≥60s、≥900 frames、稳定 calibration/service session ID、允许的 `da360-metric|cesium-truth` 和严格递增的 identity/timestamp；自动测试已复验，但尚未在 Firefox+GPU 上证明 15Hz。

## Live API verified; interactive flight pending

- 旧的启动期模型/配置/镜像内容哈希绑定已撤除；现在核对可读版本、本地路径、模型文件名、配置名与 checkpoint coverage。
- live `/health` 与 `/yopo/health` 为 API v2/CUDA，DA360 bicubic、0.459、checkpoint coverage 100%；YOPO 以可读 base/overlay 配置名报告运行配置并校验参数覆盖率。
...
- 六次 Cesium scene render、跨 context face upload 默认每两面一次 rAF yield；3 面方案因真实飞行日志中单帧阻塞加重而回退为显式 A/B。规划安装已与2s低频的缓存深度预览获取/解码解耦。真实 Firefox preview A/B 中，新默认两次 capture p95=53/45ms、RAF/physics-frame p95=17.1/17.1ms，优于旧流式对照的62/33.1ms；但这仍不是带航点的15Hz闭环验收。
...
- 50 次旧的未授权 relative 全链诊断显示服务 p95 40.7ms、HTTP p95 76.1ms，但该路径现已 fail closed，不能计作有效 planning。真实 Firefox preview A/B 已证明默认 1200m far + lean streaming 两次达到 capture p95 53/45ms、RAF p95 17.1/17.1ms，优于旧流式对照 62/33.1ms；它仍不是带航点的平均 planning ≥15Hz、p95 planning 间隔与 capture-to-apply 正式证据。
- 尚未完成直线街谷、转角避障、取消/换目标的 5 分钟真实飞行。

正式 evaluator v2 还要求 schema v2、30 warmup、60s/900 计量 frames、实际轨迹回执、唯一 identity、稳定 calibration ID/service session ID。每份日志声明单一 `navigationSession(goalId,generation)`，generation 必须是非负 JavaScript-safe integer，所有 planning frame 必须匹配；日志还需有有限 `monotonicStartMs`、正数 `duration_s`，全部合法 apply 时间位于 session 起止边界内（终点只允许 5.1ms 取整余量），且第一次/最后一次合法 apply 距相应 session 边界均不超过 `250+5.1ms`。任何 applied/`trajectoryApplied=true` 声称都必须与 `planningAuthorized=true` 一致，blocked/rejected/unauthorized 不得声称已安装轨迹。physics 时间戳须覆盖至少 95% 计量窗口，最少帧数为 `ceil(measurementDurationMs×0.95/33.3)+1`，所以 60s 窗口至少 1713 帧。relative preview 即使频率高也必须失败。飞行日志的 `resolvedUrl` 仅接受本机 `127.0.0.1|localhost` 的 `/|/index.html` 入口，保留已审查的全景/性能参数白名单（包含 5 个实际生效的 `flightPreload*` 参数），端点 URL、未知参数与凭据均不记录；evaluator 对该字段再做独立、失败关闭的 URL/schema 校验。

### 依赖与发布收尾

- `dependencies.versions.json` 记录本地依赖版本、来源、路径、checkpoint 文件名与容器 tag，不含内容哈希。
...
页面重新就绪后执行 `window.__getPanoramaCaptureProfile()`，预期为 `flight`。`window.__setPanoramaCaptureProfile('flight')` 会取消在途 capture，但只改变当前运行态；未清理 legacy URL 时，下次 reload 仍会回到 calibration。backend service-session 改动仍需按上面的镜像重建/identity 核对，不能只 reload 页面就视为生效。

验收报告必须分别列出：commit、工作树状态、镜像 ID、可读模型/配置名、service session ID、Cesium/PlayCanvas 版本、场景、预热方式、原始 flight log 和 p50/p95 指标。
