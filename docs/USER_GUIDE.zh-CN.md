...
系统会把点击取得的水平坐标与当前或滚轮指定高度组合为最终三维目标，并在该水平位置查询已解析表面高度。最终目标高度必须高于“表面高度 + 无人机碰撞半径 + 0.2 m”；落在建筑内部、建筑表面、净空不足，或目标处几何/表面高度尚未解析时都会被拒绝，不会创建目标标记或提交规划。

左下角深度小地图也可用鼠标左键单击设置航点，无需按住 <code>G</code>。小地图使用同一目标高度和三维净空校验：若当前已通过 <code>G</code> + 滚轮指定高度，则使用该高度，否则沿用无人机当前高度。单击仍须落在圆形范围内并依赖新鲜深度；相对深度模式下的圆形单击仍仅用于交互测试，不能授权 YOPO 轨迹。
