...
Firefox 和 Chrome 的 <code>localStorage</code> 相互独立，因此需要分别设置。该配置流程不会把 token 写入应用页面 URL、浏览器启动命令或应用主动日志。不要把真实 token 写进 README、启动脚本、URL 截图或飞行日志；旧版本中曾经公开的 token 必须在 Cesium Ion 后台撤销并重新创建受限 token。
...
Firefox 适合主画面、G 航点和完整 Demo 测试。启动器固定使用独立、持久、非隐私的项目 profile；可用 <code>FIREFOX_PROFILE_DIR</code> 覆盖默认目录。
...
1. 切换到 SO3 并完成 Arm。
2. 按住 <code>G</code>。
3. 点击已解析的地面、道路、建筑屋顶或立面；点击位置只用于取得水平坐标，不会因为表面属于建筑物而直接被拒绝。
4. 目标默认沿用无人机当前高度；需要改变高度时，可在按住 <code>G</code> 时滚动鼠标滚轮，再点击目标表面。
5. 松开 <code>G</code> 后观察目标标记和 planning 状态。
6. 按 <code>C</code> 可取消当前导航。

系统会把点击取得的水平坐标与当前或滚轮指定高度组合为最终三维目标。运行时先在该水平位置查询已加载表面高度，要求目标高于“表面高度 + 无人机碰撞半径 + 0.2 m”，再从目标中心沿水平八方向和竖直上下方向，对当前可查询渲染几何执行同一距离范围的近似探测。该探测只会发现射线实际截获的当前渲染几何，存在方向盲区，也不能证明邻近瓦片已经全部加载，因此不构成完整球体净空、真实飞行安全或安全认证证明。落在已解析表面以内、探测到邻近几何，或表面/射线查询无法完成时会被拒绝，不会创建目标标记或提交规划。

左下角深度小地图也可用鼠标左键单击设置航点，无需按住 <code>G</code>。小地图使用同一目标高度、表面高度检查和十方向近似探测：若当前已通过 <code>G</code> + 滚轮指定高度，则使用该高度，否则沿用无人机当前高度。单击仍须落在圆形范围内并依赖新鲜深度；相对深度模式下的圆形单击仍仅用于交互测试，不能授权 YOPO 轨迹。

进入目标点约 4 m 半径后，系统提交完成状态。当前最后一小段 Poly5 可以执行结束，之后停止 YOPO 重规划并进入 SO3 位置保持。该项目不要求复杂的厘米级停稳判定。

## 8. 飞行历史路径和目标标记

Tab/Settings 中提供历史路径开关和 <b>Clear</b> 操作。开启后，主场景绘制无人机已经飞过的线条；关闭只影响显示，不影响轨迹采样。点击 <b>Clear</b> 会立即丢弃此前的历史轨迹，以无人机当前位置作为新的记录起点；隐藏状态下清除同样生效，重复点击也安全。清除不会改变路径显隐、Clean Mode、导航目标、控制器或 FlightLogger。G 航点标记参与正常场景深度遮挡，建筑遮挡时不会穿透显示。
