...
3. 在 SO3 模式按住 <code>G</code>，可点击已解析的地面、建筑屋顶或立面取得水平坐标；目标沿用当前高度，也可在按住 <code>G</code> 时用滚轮指定高度。系统会检查目标正下方的已加载表面，并对当前可查询渲染几何做十方向近似探测；表面高度未解析、探测调用无法完成或探测到邻近几何时均拒绝。该近似探测不能证明完整球体净空，也不能替代安全认证。左下角深度小地图可直接单击设置同规则目标，无需按 <code>G</code>。
4. 使用 <code>C</code> 取消导航；飞行历史路径可在 Tab 设置中控制显示或点击 <b>Clear</b> 从当前位置重新记录。
...
| G 航点 | 点击地面、屋顶或立面取水平坐标，按当前/滚轮高度检查已加载表面并执行十方向近似探测；左下小地图单击无需按 G |
